...
```text
app/src/main/jniLibs/arm64-v8a/libproxyt.so
```

The Android app launches that file directly from `nativeLibraryDir`.

## Open in Android Studio

1. Install an Android SDK if this machine does not already have one.
2. Run `./scripts/build-android-binary.sh`.
3. Open this folder in Android Studio.
4. Let Gradle sync and install the app onto an Android phone.
...
1. Rebuild the bundled Android Go binary.
2. Create `local.properties` if `ANDROID_HOME` or `ANDROID_SDK_ROOT` is set.
...
- This wrapper currently targets `arm64-v8a`.
