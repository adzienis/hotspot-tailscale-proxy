...
func init() {
	...
	serveCmd.Flags().StringVar(&baseURL, "base-url", "", "Public base URL to advertise to clients (for example http://192.168.43.1:8080)")
...
func runProxy() {
	...
	baseURL = strings.TrimRight(viper.GetString("base-url"), "/")
	...
	if !httpOnly && domain == "" {
		logger.Fatal("domain is required when not using --http-only mode")
	}
	if !httpOnly && certDir == "" {
	...
	if baseURL != "" {
		...
		if err != nil {
			logger.Fatal("base-url must be a valid URL", zap.Error(err))
		}
		if u.Scheme != "http" && u.Scheme != "https" {
			logger.Fatal("base-url must use http or https", zap.String("base_url", baseURL))
		}
		if u.Host == "" {
			logger.Fatal("base-url must include a host", zap.String("base_url", baseURL))
		}
	}
	...
	reverseProxy := &httputil.ReverseProxy{
		Director: func(req *http.Request) {
			...
			req.Header.Set("X-Proxyt-Original-Host", req.Host)
			req.Header.Set("X-Proxyt-Original-Proto", requestScheme(req))
		...
		ModifyResponse: func(resp *http.Response) error {
			...
			if location := resp.Header.Get("Location"); location != "" {
				advertiseURL := advertisedBaseURL(resp.Request)
				if newLocation := rewriteTailscaleURLForBase(location, advertiseURL); newLocation != location {
			...
			if strings.Contains(contentType, "application/json") ||
				...
				rewrittenBody := rewriteTailscaleURLsInBody(string(body), advertisedBaseURL(resp.Request))
...
func rewriteTailscaleURL(url string) string {
	return rewriteTailscaleURLForBase(url, baseURL)
}

// rewriteTailscaleURLsInBody rewrites Tailscale URLs in response body content
func rewriteTailscaleURLsInBody(body string, advertiseURL string) string {
	...
	body = tailscaleURLRegex.ReplaceAllStringFunc(body, func(match string) string {
		return rewriteTailscaleURLForBase(match, advertiseURL)
	})
	...
	body = protocolRelativeRegex.ReplaceAllStringFunc(body, func(match string) string {
		return "//" + advertisedHost(advertiseURL)
	})
...
func rewriteTailscaleURLForBase(rawURL, advertiseURL string) string {
	if advertiseURL == "" {
		advertiseURL = defaultAdvertisedBaseURL()
	}

	rawURL = strings.Replace(rawURL, "https://login.tailscale.com", advertiseURL, -1)
	rawURL = strings.Replace(rawURL, "https://controlplane.tailscale.com", advertiseURL, -1)
	rawURL = strings.Replace(rawURL, "http://login.tailscale.com", advertiseURL, -1)
	rawURL = strings.Replace(rawURL, "http://controlplane.tailscale.com", advertiseURL, -1)

	host := advertisedHost(advertiseURL)
	rawURL = strings.Replace(rawURL, "//login.tailscale.com", "//"+host, -1)
	rawURL = strings.Replace(rawURL, "//controlplane.tailscale.com", "//"+host, -1)
...
func defaultAdvertisedBaseURL() string {
	if baseURL != "" {
		return baseURL
	}
	scheme := "https"
	...
	if httpOnly {
		scheme = "http"
		if port != "80" && port != "" && !strings.Contains(host, ":") {
			host = host + ":" + port
		}
	} else if httpsPort != "443" && httpsPort != "" && !strings.Contains(host, ":") {
		host = host + ":" + httpsPort
	}
	if host == "" {
		return ""
...
func advertisedBaseURL(req *http.Request) string {
	if baseURL != "" {
		return baseURL
	}

	scheme := req.Header.Get("X-Proxyt-Original-Proto")
	if scheme == "" {
	...
	host := req.Header.Get("X-Proxyt-Original-Host")
	if host == "" {
		host = req.Header.Get("X-Forwarded-Host")
	}
	if host == "" {
		return defaultAdvertisedBaseURL()
	}

	return scheme + "://" + host
}

func advertisedHost(advertiseURL string) string {
	if advertiseURL == "" {
		return domain
	}
